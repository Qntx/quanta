...
`Data` 类采用 **NumPy 二维数组** 存储 TOHLCV（Timestamp, Open, High, Low, Close, Volume）数据，具有高效、固定容量的特点，非常适合高频金融数据管理。  

!!! note "核心特点"  
    - **预分配容量**：通过初始化参数 `capacity` 确定大小，数据超出容量时自动 **删除最旧数据** 或 **扩展容量**。  
    - **高效访问**：支持通过只读视图 (`view`) 进行列、行或切片访问，简化数据操作并提升性能。  

!!! note "数据细节"  
    - **数组形状**：默认形状为 `(capacity, 6)`，`capacity` 由用户指定，每行存储一个完整的时间点数据。  
    - **列定义**：顺序为 `timestamp`, `open`, `high`, `low`, `close`, `volume`，数据类型为 `np.float64`。  
    - **数据操作**：支持行、列切片及高效数值计算，基于 NumPy 的底层优化实现。  
